...
# is creating _init_.py files in both the management and commands
...
deserunt mollit anim id est laborum."""[init:end]
    ...
    def addCategory(self, noCategories):
        ...
        for x in range (noCategories):
            categoria = Category()
            categoria.name = "category " + str(x)
            categoria.tooltip="tooltip , ni idea"
            categoria.save()
        # create 5 categories <<<<<<<<<<<<<<<<<<<<<<<
    ...
    def addWorkflow(self, noWorkflows):
        ...
        x=0
        for x in range(noWorkflows):
            workflow=Workflow()
            workflow.name = "workflow "+str(x)

            workflow.description="Esta es la descripcion"
            workflow.views=3
            workflow.downloads=2
            workflow.versionInit="1.0"
            workflow.keywords="palabras clave"
            workflow.json =self.getJson()
            workflow.save()
            cat = categoria[randint(0,4)]
            CategoriesAmount.objects.create(workflow = workflow, categories = cat)
            #workflow[x].category.add(cat)
            cat2 = categoria[randint(0,4)]
            while cat2 == cat:
                cat2 = categoria[randint(0,4)]
            CategoriesAmount.objects.create(workflow = workflow, categories = cat2)
            #workflow[x].category.add(cat)
            workflow.save()
    ...
    },
    {
        ...
        "numberOfThreads": 4,
        "numberOfMpi": 1,
        "inputMovies": "2._attribute_outputMovies"
    },
    {
        ...
        "numberOfThreads": 4,
        "numberOfMpi": 1,
        "inputMicrographs": "40._attribute_outputMicrographs"
    },
    {
    ...
    },
    {
        ...
        "inputObject": "82._attribute_outputCTF"
    },
    {
    ...
    },
    {
        ...
        "numberOfThreads": 1,
        "numberOfMpi": 1,
        "ctfRelations": "82._attribute_outputCTF",
